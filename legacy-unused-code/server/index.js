...
io.on("driveCommands", (driveCommands) => {
  if (!driveSerial.isOpen) return;

  let xVel = parseInt(driveCommands["xVel"], 10)
    .toString(16)
    .slice(-4)
    .padStart(4, "0");
  let yVel = parseInt(driveCommands["yVel"], 10)
    .toString(16)
    .slice(-4)
    .padStart(4, "0");
  let rotVel = parseInt(driveCommands["rotVel"], 10)
    .toString(16)
    .slice(-4)
    .padStart(4, "0");
  ...
  driveSerial.write(
    encoder.encode(
      "t" + ID.SET_CHASSIS_VELOCITIES + "6" + vXvel + yVel + rotVel + "\r",
    ),
  );
});
